...
const RecentOrderSection: React.FC<RecentOrderSectionProps> = ({ recentOrder, isLoading }) => {
  if (isLoading) {
    return (
      <View className="px-5 mb-8">
        ...
        <View className="bg-white p-6 rounded-2xl shadow-md items-center justify-center h-32">
  ...
  if (!recentOrder) {
    return (
      <View className="px-5 mb-8">
        <Text className="text-2xl font-poppins-bold text-herb-primaryDark mb-4">Recent Order</Text>
        <View className="bg-white p-8 rounded-2xl shadow-md items-center justify-center">
          <LinearGradient
            colors={['rgba(224, 231, 223, 0.7)', 'rgba(192, 210, 186, 0.5)']}
            className="w-16 h-16 rounded-full items-center justify-center mb-3"
          >
            <Ionicons name="bag-handle-outline" size={32} color="#2B4D3F" />
          </LinearGradient>
  ...
  return (
    <View className="px-5 mb-8">
      <Text className="text-2xl font-poppins-bold text-herb-primaryDark mb-4">Recent Order</Text>
      <View className="bg-white rounded-2xl shadow-md overflow-hidden border border-herb-divider/70">
        <View className="p-4 border-b border-herb-divider/70">
          <View className="flex-row items-center justify-between">
            <View className="flex-row items-center">
              ...
              >
                <Ionicons name="receipt-outline" size={22} color="#2B4D3F" />
              </LinearGradient>
              <View>
                ...
                <View className="flex-row items-center">
                  <Ionicons name="calendar-outline" size={12} color="#5F6F64" />
        ...
        <View className="p-4">
          <View className="flex-row items-center justify-between p-3 bg-herb-surface/70 rounded-xl mb-4">
            <View className="flex-row items-center">
              <Ionicons name="cube-outline" size={18} color="#3E6643" />
          ...
          <View className="mb-4">
            {recentOrder.items?.slice(0, 3).map((item, index) => (
              <View 
                key={item.id + index} 
                className="flex-row items-center mb-2 p-2 bg-herb-surface/40 rounded-lg"
              >
                {item.imageUrl ? (
                  <Image
                  ...
                  <View className="w-8 h-8 rounded-md mr-3 bg-herb-primaryLight/10 items-center justify-center">
                    <Ionicons name="leaf-outline" size={16} color="#3E6643" />
                  </View>
                )}
                <Text className="text-herb-primaryDark text-sm font-poppins-medium flex-1" numberOfLines={1}>
                  {item.name}
                </Text>
                <Text className="text-herb-primary text-xs font-poppins-semibold bg-herb-primaryLight/10 px-2 py-1 rounded-full">
                  {item.quantity}×
                </Text>
              </View>
            ))}
            ...
            {recentOrder.items?.length > 3 && (
              <View className="bg-herb-primaryLight/10 p-2 rounded-lg mb-2 flex-row items-center justify-center">
                <Ionicons name="ellipsis-horizontal" size={16} color="#3E6643" />
          ...
          <Link href={`/order/${recentOrder.id}`} asChild>
            <Pressable className="bg-herb-primary py-3.5 rounded-xl items-center shadow-md active:bg-herb-primaryDark">
